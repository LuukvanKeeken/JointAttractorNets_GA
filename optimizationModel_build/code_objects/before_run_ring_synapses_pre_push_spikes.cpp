...
double* const _array_ring_synapses_delay = _dynamic_array_ring_synapses_delay.empty()? 0 : &_dynamic_array_ring_synapses_delay[0];
const size_t _numdelay = _dynamic_array_ring_synapses_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_ring_neurons__spikespace = _array_ring_neurons__spikespace;
    double* __restrict  _ptr_array_ring_synapses_delay = _array_ring_synapses_delay;

    std::vector<double> &real_delays = _dynamic_array_ring_synapses_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = ring_synapses_pre.sources.empty() ? 0 : &(ring_synapses_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = ring_synapses_pre.sources.size();
    ring_synapses_pre.prepare(_n_sources,
                           _n_targets,
