...
const size_t _numt = 1;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double V = _ptr_array_ring_neurons_V[_idx];
        const char not_refractory = _ptr_array_ring_neurons_not_refractory[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (V > Vth) && false;
        else 
        ...
        if(_cond) {
            _ptr_array_ring_neurons__spikespace[_count++] = _idx;
            _ptr_array_ring_neurons_not_refractory[_idx] = false;
            _ptr_array_ring_neurons_lastspike[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_ring_neurons__spikespace[N] = _count;
